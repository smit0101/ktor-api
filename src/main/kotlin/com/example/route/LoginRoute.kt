...
fun Route.loginRoute() {
    get("/user/{email?}") {
        ...
        val user = getCollection<User>().findOne(User::email eq email) ?: return@get call.respondText(
            "faild",
...
fun Route.onePlaceRoute() {
    get("/websites") {
        ...
        val webSites = webSiteCollection.find().toList() as? MutableList ?: return@get call.respondText(
            "failed",
            status = HttpStatusCode.NotFound
        )
    ...
    post("/add/websites") {
        ...
        if (result) call.respond(
            onePlaceDb.getCollection<Website>().find().toList() as? MutableList ?: mutableListOf<Website>()
        ) else return@post call.respondText("failed", status = HttpStatusCode.NotFound)
    }
    delete("/remove/website/{title?}") {
        val title =
            call.parameters["title"] ?: return@delete call.respondText("failed", status = HttpStatusCode.NotFound)
        val result = onePlaceDb.getCollection<Website>().deleteOne(Website::title eq title).wasAcknowledged()
        if (result) call.respond(
            onePlaceDb.getCollection<Website>().find().toList() as? MutableList ?: mutableListOf<Website>()
        ) else return@delete call.respondText("failed", status = HttpStatusCode.NotFound)
    }
}